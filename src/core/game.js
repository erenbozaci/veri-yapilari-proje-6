...
    // Navigasyon Grafı
    this.navGraph = new Graph();
    const cellSize = 50; 

    for (let x = 75; x < this.width - 50; x += cellSize) {
        for (let y = 75; y < this.height - 50; y += cellSize) {
            this.navGraph.addNode(`${x}_${y}`, x, y);
        }
    }

    for (let [id1, p1] of this.navGraph.nodes.entries()) {
        for (let [id2, p2] of this.navGraph.nodes.entries()) {
            if (id1 === id2) continue;
            const dx = Math.abs(p1.x - p2.x);
            const dy = Math.abs(p1.y - p2.y);
            if ((dx === cellSize && dy === 0) || (dx === 0 && dy === cellSize)) {
                this.navGraph.addEdge(id1, id2);
            }
        }
    }
...
                if (dist < 18) { enemy.path.shift(); } else { eMoveX = dx / dist; eMoveY = dy / dist; }
            }
        }
...
  // --- 🚨 FENER IŞIĞI MESAFESİ 260'TAN 140'A DÜŞÜRÜLDÜ (DAHA GERÇEKÇİ/YAKIN) ---
...
    
    // Işık mesafesini 140 piksele düşürerek daha dengeli yaptık kral
...
    // Gradyan bitiş noktasını da yeni mesafe olan 140'a eşitledik kral
...
    // --- 🎓 AKADEMİK VE RESMİ DİL GÜNCELLEMESİ (HOCANIN SEVECEĞİ TARZ) ---
...
        this.ctx.fillText("DÜŞMANA YAKALANDINIZ!", this.width / 2, this.height / 2 - 20);
        
        this.ctx.fillStyle = "#ffffff";
        this.ctx.font = "20px Arial";
        this.ctx.fillText("Yeniden denemek için klavyeden F5 tuşuna basınız.", this.width / 2, this.height / 2 + 50);
    }
...
        this.ctx.font = "bold 54px Arial";
        this.ctx.textAlign = "center";
        this.ctx.fillText("HEDEF NOKTAYA ULAŞTINIZ!", this.width / 2, this.height / 2 - 20);
        
        this.ctx.fillStyle = "#ffffff";
        this.ctx.font = "20px Arial";
        this.ctx.fillText("Başarıyla tamamlandı. Yeni bir harita üretmek için F5 tuşuna basınız.", this.width / 2, this.height / 2 + 50);
    }
  }
} 